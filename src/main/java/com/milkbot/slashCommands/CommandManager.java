...
import com.milkbot.slashCommands.dev.ClearInstances;
import com.milkbot.slashCommands.dev.ResetUsersHWID;
import com.milkbot.slashCommands.fun.EmoteCommand;
import com.milkbot.util.NumberFormatter;
import com.milkbot.slashCommands.utility.PollCommand;
import com.milkbot.slashCommands.utility.ResetHWID;
import com.milkbot.util.embeds.EmbedUtils;

import com.milkbot.slashCommands.dev.DeleteAccount;
import com.milkbot.slashCommands.dev.CreateAccount;
...
public class CommandManager extends ListenerAdapter {
    ...
    private final String discordIDforDevAccess = "405043391135612929";
    private final String discordDevName = "bobby";


    public CommandManager(MilkBot bot) {
        this.bot = bot;
        mapCommands(
                // Utility Commands
                new PollCommand(bot), new ResetHWID(bot),
                // Fun Commands
                new EmoteCommand(bot),
                // Dev only
                new CreateAccount(bot),new ClearInstances(bot), new DeleteAccount(bot), new ResetUsersHWID(bot));
    }
...
//        bot.getDatabase().createUserData(event.getUser());
        ...
        if (cmd != null) {
            if (cmd.devOnly && !event.getUser().getId().equals(discordIDforDevAccess)) {
                event.replyEmbeds(EmbedUtils.createError("This is a **"+discordDevName+"** command")).setEphemeral(true).queue();
                return;
            }
